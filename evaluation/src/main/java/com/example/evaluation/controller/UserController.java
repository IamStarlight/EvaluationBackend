...
public class UserController {
    ...
    public ResponseEntity<Result> logout(){
        return new ResponseEntity<>(Result.success(service.logout()), HttpStatus.OK);
    }
    ...
    public ResponseEntity<Result> updateUserPwd(@RequestParam Integer id,
                                                ...
                                                String newpwd){
        return new ResponseEntity<>(Result.success(service.updateUserPwd(id,newpwd)), HttpStatus.OK);
    }

    //用户修改自己的密码
    // TODO: 2023-11-06 权限不足 
    ...
    public ResponseEntity<Result> updateMyPwd(@CurrentUser User user,
                                              ...
                                              @Valid @NotBlank(message = "新密码不能为空")
                                              String newpwd){
        return new ResponseEntity<>(Result.success(service.updateUserPwd(user.getId(),oldpwd,newpwd)), HttpStatus.OK);
    }
}
